...
- Missing bootstrap for a brand-new graph: <a name='id/BlockNode:00CP9R475R001' class='aperas-anchor aperas-id'></a> `rehydrateStore` (`store.ts`) treats a missing `Apeiron/*.jsonld` instance file as `ENOENT`, not an empty graph, so a fresh directory with no store files at all crashed the service on startup — surfacing only as `aperas serve`/`aperas service start`'s generic "did not become ready in time", with nothing pointing at the real cause. Found live installing the published `aperas` package into a brand-new project. Fixed with a new `aperas init` command that creates `aperas.config.json`, the four empty instance files, and `artifacts/` — see [history](../history/packaging.md#id/BlockNode:00CP9QYPNG000).
- No skill counterpart for an installed `aperas`: <a name='id/BlockNode:00CP9R475R002' class='aperas-anchor aperas-id'></a> this project's own `aperas`/`kg-doc-ingest` skills only ever lived in this repo's `.claude/skills/`, so an agent working against a plain `npm install -g aperas` had the CLI with none of the graph-first discipline the skill encodes. Fixed by bundling both skills into the published package (`dist/skills/`) and adding `aperas skill install` to copy them into an agent's own `.claude/skills/` — see [history](../history/packaging.md#id/BlockNode:00CP9QYPNG001).
