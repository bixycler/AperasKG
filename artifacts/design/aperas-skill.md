...
Only current, verified items appear: <a name='id/BlockNode:00CEA5Y5JG00C' class='aperas-anchor aperas-id'></a> a rule whose cause has since been removed, and one identified but not yet confirmed live, both stay in `discussion/aperas-skill.md` instead. A caution whose cause has been removed is worse than no caution, because it still costs the reader something and no longer buys anything — and an inline status vocabulary makes every reader of every item pay to mark the rare exception, where the snapshot-and-delta thread already records the same distinction more precisely and without touching the rules themselves.

The document's own past stays out: <a name='id/BlockNode:00CGF54B5G001' class='aperas-anchor aperas-id'></a> prior wordings, a section's retired name, tool behaviour that has since changed, and version history all belong in `discussion/aperas-skill.md`. This is distinct from [the originating incident above](#id/BlockNode:00CEA5Y5JG00B), which is provenance rather than history — it explains why a rule that is true now is true, and so travels with the rule. The test is whether a sentence still reads to someone with no memory of the version that preceded it, and a correction appended to text left unfixed always fails it. That is why the repair is to rewrite the item until it is complete on its own, never to leave it standing and add a paragraph explaining what the earlier one got wrong.
...
The concerns form an abstraction gradient, `design` most abstract and `discussion` least, with `issues`, `planning` and `history` between. A citation may point up that gradient or sideways along it freely. It may not point down: a design block does not reference a discussion block, for the same reason a node carries a parent pointer and not a list of children. The concern taxonomy's own `# Context` convention ([documentation](../design/documentation.md#id/BlockNode:00CDD68E8000K): "one link plus a brief, non-argumentative summary per doc") already states this same up-only citation shape for its own instance of that exception, independently of this rule.

The sanctioned exception is a **normative, singular** pointer — one whose target is the block's whole referent rather than one of many possible mentions. A design doc's own `# Context` section is the familiar instance, one link per facet over a fixed set, but it is the *property* that is sanctioned rather than that location: a history milestone announcing one snapshot, or a design block naming *the* canonical exemplar, qualify the same way. Reflects [v1.1](../discussion/aperas-skill.md#id/BlockNode:00CF5GK5BR003).

The test is whether removing the link leaves the block **incomplete**. A milestone whose content is "this snapshot was taken" no longer says what it exists to say once its pointer is gone, while "here is a discussion that also touched on this" loses only enrichment and stays forbidden. What the rule is actually against is a *list of children* — unbounded, discretionary, accumulating — so cardinality is the real criterion and direction is only its usual symptom. Reflects [v1.1](../discussion/aperas-skill.md#id/BlockNode:00CF5GK5BR004).

### Entry route — both directions, and the gap where they meet <a name='id/BlockNode:00CN7XM9TG001' class='aperas-anchor aperas-id'></a>

The gradient that constrains citation is also the route for reading along it, and both directions are legitimate. Top-down — `design`, then `issues`/`history`, then `discussion`, then the code — narrows like a binary search and yields the frame: what the corpus claims about an area, and which facet the work is actually in. Bottom-up — a concrete identifier grepped against the store, landing on the exact node, then climbing outward — yields the pinpoint, which a top-down descent cannot supply without first choosing among dozens of sibling entries. Neither is a fallback for the other.

Drift between a tier and what it governs is detectable only where the two directions meet, because it is an absence as often as a contradiction — the abstract tier silent where the concrete has a rule, or asserting one the concrete has since replaced — and in both forms it is indistinguishable from agreement until something has been compared against it. Descending alone confirms the abstract; ascending alone confirms the concrete. Which axis carries the climb follows from citation direction: a lower block cites the higher one it realizes, so forward links usually run up the gradient, while a higher block points down only where it speaks normatively; backlinks are that same relation from the other end, and are how an abstract block's realizers below are found; ancestors climb within one artifact only, recovering the surrounding thread rather than the tier that governs it. Reflects [v2.8](../discussion/aperas-skill.md#id/BlockNode:00CN7YEND0002).
