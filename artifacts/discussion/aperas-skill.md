...
- **A mechanical read-side gate was built, tested, and broke three separate ways — none of them a config typo**: <a name='id/BlockNode:00CF80TVJG001' class='aperas-anchor aperas-id'></a> to close the trigger gap just above, a `PreToolUse` hook (`skills/aperas/scripts/aperas_gate.py`) was registered in a personal `.claude/settings.local.json` — deliberately never shared, since a hard gate baked into the project would be the "destroys the whole meaning of skill" mistake the entry above warns against. It denied `Read`/`Edit`/`Grep` under `monorepo/packages/**` and `AperasKG/**` until the session had run a real `aperas` command, tracked by a session-scoped marker file. Two rounds of live testing found three independent bugs, each confirmed rather than guessed at.
  ...
  - **The example that was tested, kept here since the file itself doesn't survive**: <a name='id/BlockNode:00CF80TVJR003' class='aperas-anchor aperas-id'></a> `.claude/settings.local.json` is personal and gitignored by design, so this is its only durable record.
    ...
    ```
  - **Why this was retired rather than kept running**: <a name='id/BlockNode:00CF80TVJR005' class='aperas-anchor aperas-id'></a> three bugs surfaced across two rounds of testing, on a mechanism that is inherently personal, machine-specific (its own fix hardcodes an absolute path), and invisible to anyone who didn't set it up themselves. [Promoted to a v1.3 delta](#id/BlockNode:00CF80MKFG001): the next attempt trades a hard mechanical gate for a soft, one-time consent question the skill asks itself, with the answer remembered durably instead of mechanically enforced.
...
- **`design/aperas-skill.md`'s Citation direction sat thirteen versions behind its own projection, and none of the three existing instruments could have caught it**: <a name='id/BlockNode:00CN6S7HWR000' class='aperas-anchor aperas-id'></a> v1.1 generalized the sanctioned exception from "a designated index" to a **normative, singular** pointer, tested by whether removing the link leaves the block incomplete — cardinality is the real criterion and direction only its usual symptom. That landed in SKILL.md and was recorded as a delta ([v1.1](#id/BlockNode:00CF5GK5BR002)), but the design block it revised ([Citation direction](../design/aperas-skill.md#id/BlockNode:00CEA5Y5JG00H)) was never updated, and still asserted the pre-v1.1 wording at v2.7 — read top-down and quoted back as current before the gap was noticed. Measured while finding it: of design's 50 live blocks, 43 carry v1-authoring ids, 6 are v1.x-era and 1 is v2-era, with none after — additions land, revisions strand. The cause is structural rather than disciplinary: the edit loop's deep-write step answers "what did this change make stale?" by backlinks, and SKILL.md is a generative projection with no node and therefore no backlinks, so on the design ↔ SKILL.md edge that step has nothing to traverse. All three instruments sit on the other edge — `skill_drift.py` compares SKILL.md against the snapshot/delta thread, the [vertical thread](#id/BlockNode:00CF4D9XT0001) wires principles to their realizers *inside* the v1 snapshot and was never rebuilt past v1.2, and the snapshot/delta record is SKILL.md's own history rather than design's. `aperas backlinks` on the drifted block returned nothing at all. Remedy adopted: a version delta that revises a rule links **up** to the design block it revises — upward and so legal by citation direction, singular and constitutive and so sanctioned by that same rule's own test — which makes a design block's backlinks its status check, a backlink from a delta newer than the block's last edit being the staleness signal, and no backlinks at all meaning no version has yet revised it. Promoted to [issues/aperas-skill.md](../issues/aperas-skill.md#id/BlockNode:00CN7G6D0G001).
...
Folded into design: [Citation direction](../design/aperas-skill.md#id/BlockNode:00CEA5Y5JG00H) and [its incompleteness test](../design/aperas-skill.md#id/BlockNode:00CN6SMSX0001).
...
Supersedes: [v2.7's own status line](#id/BlockNode:00CJ5207P8003)

Status: <a name='id/BlockNode:00CN7ZQFN8002' class='aperas-anchor aperas-id'></a> **v2.8** — four levels, Philosophy through Mechanics, each item explaining a consequence of the one above it. Only current, verified items appear here; superseded material, unverified hypotheses and version-by-version rationale live in `discussion/aperas-skill.md`'s snapshot and deltas. Concern docs: `AperasKG/artifacts/{design,issues,planning,history,discussion}/aperas-skill.md`.

### Orientation — entering the corpus, two directions <a name='id/BlockNode:00CN7YEND0002' class='aperas-anchor aperas-id'></a>
...
### Discipline — the code-entry recipe gains its return leg <a name='id/BlockNode:00CN7YEND0003' class='aperas-anchor aperas-id'></a>
