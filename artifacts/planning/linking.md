...
1. **Fold Slice 1's scratch-ingest verification into `verify.ts`** — landed as `verify.ts`'s step 13, following its existing pattern (`DEMO_DIR` scratch artifacts, thrown-`Error` assertions).
...
3. **The cleanup pass** — landed as documented: <a name='id/BlockNode:00CDBZ77F0014' class='aperas-anchor aperas-id'></a> no new removal API, since none was needed.
