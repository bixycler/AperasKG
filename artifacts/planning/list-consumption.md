...
- [x] `astParser.ts` normalizes props onto the run's own first item and splices orphaned lists flat.
- [x] `project.ts` reads props per-run-leader and detects boundaries by type-or-own-explicit-props.
- [x] Both synthetic multi-run cases (separated, zero-separator) are covered in `verify.ts`.
- [x] The real corpus (48 legacy anchors, 59 live `list` nodes) is migrated, verified byte-identical except one approved, understood exception (see [Issues](../issues/list-consumption.md#id/BlockNode:00CE32GRZG007)).
- [x] `verify.ts`'s old hard-asserted tests are rewritten.
- [x] The two `insert`-into-a-list-target gotchas are reassessed — one moot (no live `list` node left to mistakenly target), one reclassified as a general, unrelated parser-input caveat, not a list-consumption bug.
- [x] A separate post-migration regression found afterward — `aperas insert` always minting a fresh run-leader prop on a lone piped bullet — is now also fixed (see [Issues](../issues/list-consumption.md#id/BlockNode:00CE32GRZG002)).
